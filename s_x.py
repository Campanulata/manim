...
class s_x(Scene):
    def construct(self):
        ...
        xArrowTip = ArrowTip(color = WHITE)
        xArrowTip.rotate(TAU/2)
        xArrowTip.shift(7.2*RIGHT)
        yArrowTip = ArrowTip(color = WHITE)
        yArrowTip.rotate(3/4*TAU)
        yArrowTip.shift(0.25*RIGHT+3.9*UP)
        xy = VGroup(xArrowTip,yArrowTip)
        self.play(Write(xy))
        xArrowTipt = TextMobject("t")
        xArrowTipt.shift(6.5*RIGHT+0.25*DOWN)
        yArrowTipv = TextMobject("v")
        yArrowTipv.shift(0.25*LEFT+3.5*UP)
        xyvt = VGroup(xArrowTipt,yArrowTipv)
        self.play(Write(xyvt))
        self.wait(5)
